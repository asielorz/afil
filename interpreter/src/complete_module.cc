...
namespace complete
{
	...
	auto add_type(Module & module, Type new_type) noexcept -> TypeId
	{
		unsigned const type_index = static_cast<unsigned>(module.types.size());
		module.types.push_back(std::move(new_type));
		return TypeId::with_index(type_index);
	}

	auto type_with_id(Module const & module, TypeId id) noexcept -> Type const &
	{
		assert(!id.is_function && id.index < module.types.size());
		return module.types[id.index];
	}

	auto type_size(Module const & module, TypeId id) noexcept -> int
	{
		if (id.is_reference)
			return sizeof(void *);
			...
			return type_with_id(module, id).size;
	}

	auto type_alignment(Module const & module, TypeId id) noexcept -> int
	{
		if (id.is_reference)
			return alignof(void *);
		else if (id.is_function)
			...
			return type_with_id(module, id).alignment;
	}
	...
	auto is_default_constructible(TypeId type_id, Module const & module) noexcept -> bool
	{
		if (type_id.is_function)
			return false;

		Type const & type = type_with_id(module, type_id);

		if (Struct const * const struct_data = struct_for_type(module, type))
			return is_default_constructible(*struct_data);
		else if (Type::Array const * array = try_get<Type::Array>(type.extra_data))
			return is_default_constructible(array->value_type, module);
		else
			return false;
	}
	...
	auto synthesize_default_constructor(TypeId type_id, Type::Array array_data, Module const & module) noexcept -> expression::Constructor
	{
		expression::Constructor default_constructor_node;
		default_constructor_node.constructed_type = type_id;
		default_constructor_node.parameters.reserve(array_data.size);
		default_constructor_node.parameters.push_back(synthesize_default_constructor(array_data.value_type, module));
		for (int i = 1; i < array_data.size; ++i)
			default_constructor_node.parameters.push_back(default_constructor_node.parameters[0]);
		return default_constructor_node;
	}

	auto synthesize_default_constructor(TypeId type_id, Module const & module) noexcept -> expression::Constructor
	{
		Type const & type = type_with_id(module, type_id);
		if (is_struct(type))
			return synthesize_default_constructor(type_id, *struct_for_type(module, type));
		else if (is_array(type))
			return synthesize_default_constructor(type_id, std::get<Type::Array>(type.extra_data), module);

		declare_unreachable();
	}

	auto add_struct_template(Module & module, StructTemplate new_template) noexcept -> StructTemplateId
	{
		StructTemplateId const template_id = StructTemplateId{static_cast<unsigned>(module.struct_templates.size())};
		module.struct_templates.push_back(std::move(new_template));
		return template_id;
	}
	...
	auto struct_for_type(Module const & module, Type const & type) noexcept -> Struct const *
	{
		if (auto const s = try_get<Type::Struct>(type.extra_data))
			return &module.structs[s->struct_index];
		else
			return nullptr;
	}

	auto struct_for_type(Module const & module, TypeId type) noexcept -> Struct const *
	{
		return struct_for_type(module, type_with_id(module, type));
	}
	...
	auto pointer_type_for(TypeId pointee_type, Module & module) noexcept -> TypeId
	{
		...
		for (Type const & type : module.types)
			if (auto const pointer = try_get<Type::Pointer>(type.extra_data))
				if (pointer->value_type == pointee_type)
					return TypeId::with_index(static_cast<unsigned>(&type - module.types.data()));
		...
		return add_type(module, std::move(new_type));
	}
	...
	auto pointee_type(TypeId pointer_type_id, Module const & module) noexcept -> TypeId
	{
		return pointee_type(type_with_id(module, pointer_type_id));
	}
	...
	auto array_type_for(TypeId value_type, int size, Module & module) noexcept -> TypeId
	{
		...
		Type const & value_type_data = type_with_id(module, value_type);
	...
	auto array_value_type(TypeId array_type_id, Module const & module) noexcept -> TypeId
	{
		return array_value_type(type_with_id(module, array_type_id));
	}
	...
	auto array_pointer_type_for(TypeId pointee_type, Module & module) noexcept -> TypeId
	{
		assert(!pointee_type.is_reference); // A pointer can't point at a reference.
	...
	auto add_function(Module & module, Function new_function) noexcept -> FunctionId
	{
		FunctionId const function_id = FunctionId{0, static_cast<unsigned>(module.functions.size())};
		module.functions.push_back(std::move(new_function));
		return function_id;
	}

	auto add_function_template(Module & module, FunctionTemplate new_function_template) noexcept -> FunctionTemplateId
	{
		FunctionTemplateId const function_template_id = FunctionTemplateId{static_cast<unsigned>(module.function_templates.size())};
		module.function_templates.push_back(std::move(new_function_template));
		return function_template_id;
	}

	auto parameter_types_of(Module const & module, FunctionId id) noexcept -> std::vector<TypeId>
	{
		if (id.is_extern)
		{
			return module.extern_functions[id.index].parameter_types;
		}
		else
		{
			Function const & fn = module.functions[id.index];
	...
	auto return_type(Module const & module, FunctionId id) noexcept -> TypeId
	{
		if (id.is_extern)
			return module.extern_functions[id.index].return_type;
		else
			return module.functions[id.index].return_type;
	}

	auto is_callable_at_compile_time(Module const & module, FunctionId id) noexcept -> bool
	{
		if (id.is_extern)
			return module.extern_functions[id.index].is_callable_at_compile_time;
		else
			return module.functions[id.index].is_callable_at_compile_time;
	}

	auto type_for_overload_set(Module & module, OverloadSet overload_set) noexcept -> TypeId
	{
		TypeId new_type_id;
		new_type_id.flat_value = 0;
		new_type_id.is_function = true;
		new_type_id.index = static_cast<unsigned>(module.overload_set_types.size());
		module.overload_set_types.push_back(std::move(overload_set));
		return new_type_id;
	}

	auto overload_set_for_type(Module const & module, TypeId overload_set_type) noexcept -> OverloadSetView
	{
		assert(overload_set_type.is_function);
		return module.overload_set_types[overload_set_type.index];
	}

	auto instantiate_function_template(Module & module, FunctionTemplateId template_id, span<TypeId const> parameters) noexcept -> expected<FunctionId, PartialSyntaxError>
	{
		FunctionTemplate & function_template = module.function_templates[template_id.index];
		...
		scope_stack.push_back({&module.global_scope, instantiation::ScopeType::global, 0});

		try_call_decl(Function instantiated_function,
			instantiation::instantiate_function_template(function_template.incomplete_function, all_template_parameters, scope_stack, out(module)));
		FunctionId const instantiated_function_id = add_function(module, std::move(instantiated_function));
	...
	auto instantiate_struct_template(Module & module, StructTemplateId template_id, span<TypeId const> parameters) noexcept -> expected<TypeId, PartialSyntaxError>
	{
		StructTemplate & struct_template = module.struct_templates[template_id.index];
		...
		{
			try_call_decl(TypeId const var_type, instantiation::resolve_dependent_type(var_template.type, all_template_parameters, scope_stack, out(module)));

			add_variable_to_scope(new_struct.member_variables, new_type.size, new_type.alignment, var_template.name, var_type, 0, module);
			if (var_template.initializer_expression)
			{
				try_call(assign_to(new_struct.member_variables.back().initializer_expression), instantiation::instantiate_expression(
					*var_template.initializer_expression, all_template_parameters, scope_stack, out(module), nullptr));
			}
		}

		new_type.extra_data = Type::Struct{static_cast<int>(module.structs.size())};
		module.structs.push_back(std::move(new_struct));
		...
		TypeId const new_type_id = TypeId::with_index(static_cast<unsigned>(module.types.size()));
		module.types.push_back(std::move(new_type));
	...
	auto insert_conversion_node(Expression tree, TypeId from, TypeId to, Module const & module) noexcept -> expected<Expression, PartialSyntaxError>
	{
		...
		else if (is_pointer(type_with_id(module, from)) && is_pointer(type_with_id(module, to)) && !from.is_reference && !to.is_reference)
		{
			return std::move(tree);
		}
	...
	auto insert_conversion_node(Expression tree, TypeId to, Module const & module) noexcept -> expected<Expression, PartialSyntaxError>
	{
		return insert_conversion_node(std::move(tree), expression_type_id(tree, module), to, module);
	}

	auto check_type_validness_as_overload_candidate(TypeId param_type, TypeId parsed_type, Module const & module, int & conversions) noexcept -> bool
	{
		if (param_type == parsed_type)
			return true;

		if (is_convertible(parsed_type, param_type, module))
		{
			conversions++;
			return true;
		}
		...
		Module & module) -> TypeId
	{
		auto const visitor = overload(
			...
			{
				Type const & type = type_with_id(module, given_parameter);

				if (!is_pointer(type))
					return TypeId::none; // Does not satisfy the pattern

				TypeId const pointee = pointee_type(type);
				TypeId const expected_pointee = expected_type_according_to_pattern(pointee, *pointer.pointee, resolved_dependent_types, module);

				if (expected_pointee == TypeId::none)
					return TypeId::none;
				else
					return pointer_type_for(expected_pointee, module);
			},
			...
			[&](FunctionTemplateParameterType::ArrayPointer const & pointer)
			{
				...
				if (expected_pointee == TypeId::none)
					...
					return array_pointer_type_for(expected_pointee, module);
			},
			[&](FunctionTemplateParameterType::TemplateInstantiation const & template_instantiation) -> TypeId
			{
				...
				for (size_t i = 0; i < n; ++i)
				{
					TypeId const expected_param = expected_type_according_to_pattern(
						...
						resolved_dependent_types, module);
	...
	auto resolve_function_overloading(OverloadSetView overload_set, span<TypeId const> parameters, Module & module) noexcept -> FunctionId
	{
		struct Candidate
		{
			int conversions;
		...
		{
			auto const param_types = parameter_types_of(module, function_id);
			if (param_types.size() == parameters.size())
			{
				...
				{
					if (!check_type_validness_as_overload_candidate(param_types[i], parameters[i], module, conversions))
					{
						discard = true;
						break;
					}
		...
		{
			FunctionTemplate const & fn = module.function_templates[template_id.index];
			...
			if (fn_parameters.size() == parameters.size())
			{
				...
				for (size_t i = 0; i < parameters.size(); ++i)
				{
					TypeId const expected_type = expected_type_according_to_pattern(parameters[i], fn_parameters[i], resolved_dependent_types, module);
					...
					if (!check_type_validness_as_overload_candidate(expected_type, parameters[i], module, conversions))
					{
						discard = true;
						break;
					}
		...
		else
		{
			...
			else
			{
				...
				return *instantiate_function_template(module, best_template_candidate.id, { resolved_dependent_types, dependent_type_count });
			}
		}
	}

	auto resolve_function_overloading_and_insert_conversions(OverloadSetView overload_set, span<Expression> parameters, span<TypeId const> parameter_types, Module & module) noexcept
		-> expected<FunctionId, PartialSyntaxError>
	{
		FunctionId const function_id = resolve_function_overloading(overload_set, parameter_types, module);
		if (function_id == invalid_function_id)
			return invalid_function_id;

		// If any conversion is needed in order to call the function, perform the conversion.
		auto const target_parameter_types = parameter_types_of(module, function_id);
		try_call_void(insert_conversions(parameters, parameter_types, target_parameter_types, module));
	...
	auto insert_conversions(
		...
		Module const & module) noexcept -> expected<void, PartialSyntaxError>
	{
		for (size_t i = 0; i < target_parameter_types.size(); ++i)
		{
			if (parsed_parameter_types[i] != target_parameter_types[i])
				try_call(assign_to(parameters[i]), insert_conversion_node(std::move(parameters[i]), parsed_parameter_types[i], target_parameter_types[i], module));
		}
