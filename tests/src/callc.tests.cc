...
TODO("Find a better fix than this for function pointer types")

#include "utils/compatibility.hh"
// Clang accepts the following code as microsoft extension, but only in windows
#if AFIL_MSVC || (AFIL_CLANG && AFIL_WINDOWS)
#	define CAST_FUNCPTR_TO_VOID_PTR
#else
#	define CAST_FUNCPTR_TO_VOID_PTR (void const*)
#endif
...
TEST_CASE("Function that returns an int")
{
	...
	int r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[]() { return 1293; }, nullptr, &r);
...
TEST_CASE("Function that returns a float")
{
	...
	float r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[]() { return -645.12373f; }, nullptr, &r);
...
TEST_CASE("Function that returns a char")
{
	...
	char r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[]() -> char { return 'F'; }, nullptr, &r);
...
TEST_CASE("Function that returns a double")
{
	...
	double r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[]() { return -645.12373; }, nullptr, &r);
...
TEST_CASE("Function that returns a uint64_t")
{
	...
	uint64_t r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[]() { return uint64_t(-1); }, nullptr, &r);
...
TEST_CASE("Function that returns a short")
{
	...
	short r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[]() { return 1200; }, nullptr, &r);
...
TEST_CASE("Function: (uint64_t) -> int")
{
	...
	int r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](char const * s) { return printf("%s", s); }, &str, &r);
...
TEST_CASE("Function: (float) -> int")
{
	...
	int r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](float x) { return int(x); }, &x, &r);
...
TEST_CASE("Function: (int, int) -> int")
{
	...
	int r;
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](int a, int b) { return a + b; }, &x, &r);
...
TEST_CASE("Function: (double, double) -> bool")
{
	...
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](double a, double b) -> bool { return a < b; }, &x1, &r1);
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](double a, double b) -> bool { return a < b; }, &x2, &r2);
...
TEST_CASE("Function: (char, char, char, char) -> int")
{
	...
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](char a, char b, char c, char d) -> int
	{ 
		IntBytes x;
...
TEST_CASE("Function: (ushort, ushort, ushort) -> ushort")
{
	...
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](unsigned short a, unsigned short b, unsigned short c) { return a + b + c; }, &x, &r);
...
TEST_CASE("Function: (double, ushort, int) -> double")
{
	...
	caller(CAST_FUNCPTR_TO_VOID_PTR +[](double a, unsigned short b, int c) -> double { return a + b + c; }, &x, &r);
